...
{
    private readonly SemaphoreSlim _temporalThreatSchemaGate = new(1, 1);
    ...
    private async Task EnsureTemporalThreatSchemaAsync(CancellationToken cancellationToken = default)
    {
        ...
        await _temporalThreatSchemaGate.WaitAsync(cancellationToken);
        try
        {
            ...
            cmd.CommandText =
                ...
                CREATE TABLE IF NOT EXISTS threat_campaigns_v2 (
                    ...
                    first_observed_utc TIMESTAMPTZ NOT NULL,
                    last_observed_utc TIMESTAMPTZ NOT NULL,
                    ...
                    related_incident_count BIGINT NOT NULL DEFAULT 0,
                    related_incident_ids_json JSONB NOT NULL DEFAULT '[]'::jsonb,
                    merged_into_campaign_id TEXT,
                    tombstoned_at_utc TIMESTAMPTZ,
                    involved_hosts_json JSONB NOT NULL DEFAULT '[]'::jsonb,
                    involved_ips_json JSONB NOT NULL DEFAULT '[]'::jsonb,
                ...
                CREATE TABLE IF NOT EXISTS threat_campaign_summary_history_v2 (
                    ...
                    affected_asset_count BIGINT NOT NULL,
                    related_incident_count BIGINT NOT NULL,
                    related_incident_ids_json JSONB NOT NULL,
                    merged_into_campaign_id TEXT,
                    tombstoned_at_utc TIMESTAMPTZ,
                    involved_hosts_json JSONB NOT NULL,
                    involved_ips_json JSONB NOT NULL,
                    title TEXT NOT NULL,
                ...
                INSERT INTO threat_campaign_summary_history_v2(
                ...
                SELECT tenant_id, campaign_id, revision, updated_at_utc, status, severity, confidence,
                    ...
                    merged_into_campaign_id, tombstoned_at_utc,
                    involved_hosts_json, involved_ips_json, title, summary
                FROM threat_campaigns_v2 ON CONFLICT DO NOTHING;
                CREATE OR REPLACE FUNCTION ntshield_capture_threat_campaign_history_v2()
                ...
                BEGIN
                    INSERT INTO threat_campaign_summary_history_v2(
                        ...
                        merged_into_campaign_id, tombstoned_at_utc,
                        involved_hosts_json, involved_ips_json, title, summary)
                    VALUES (NEW.tenant_id, NEW.campaign_id, NEW.revision, clock_timestamp(),
                    ...
                    ON CONFLICT (tenant_id, campaign_id, revision) DO NOTHING;
                    RETURN NEW;
                END;
                $history$ LANGUAGE plpgsql;
                DO $trigger$
                BEGIN
                    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_threat_campaign_history_v2') THEN
                        CREATE TRIGGER trg_threat_campaign_history_v2
                        AFTER INSERT OR UPDATE ON threat_campaigns_v2
                        FOR EACH ROW EXECUTE FUNCTION ntshield_capture_threat_campaign_history_v2();
                    END IF;
                END
                $trigger$;
                ...
                CREATE TABLE IF NOT EXISTS threat_contacts_v2 (
                    ...
                    updated_at_utc TIMESTAMPTZ NOT NULL,
                    observation_count BIGINT NOT NULL,
                    observation_record_count BIGINT NOT NULL,
                    recurrence_count BIGINT NOT NULL,
                    auth_failure_count BIGINT NOT NULL,
                    auth_success_count BIGINT NOT NULL,
                    ...
                    beacon_score DOUBLE PRECISION NOT NULL DEFAULT 0,
                    gap_samples_json JSONB NOT NULL DEFAULT '[]'::jsonb,
        ...
        {
            _temporalThreatSchemaGate.Release();
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        summary.TenantId = TemporalTenant(summary.TenantId);
        var first = TemporalDate(summary.FirstObservedAtUtc, summary.UpdatedAtUtc);
        var last = TemporalDate(summary.LastObservedAtUtc, first);
        var updated = TemporalDate(summary.UpdatedAtUtc, DateTimeOffset.UtcNow);
        await using var conn = new NpgsqlConnection(_cs);
        await conn.OpenAsync(cancellationToken);
        await using var cmd = conn.CreateCommand();
        cmd.CommandText =
            ...
            VALUES (@tenant, @campaign, @revision, @status, @severity, @confidence,
                ...
                @assets, @incident_count, @incident_ids::jsonb,
                @merged_into, @tombstoned,
                @hosts::jsonb, @ips::jsonb, @title, @summary)
            ON CONFLICT(tenant_id, campaign_id) DO UPDATE SET
                ...
                first_observed_utc=LEAST(threat_campaigns_v2.first_observed_utc, EXCLUDED.first_observed_utc),
                last_observed_utc=GREATEST(threat_campaigns_v2.last_observed_utc, EXCLUDED.last_observed_utc),
                updated_at_utc=GREATEST(threat_campaigns_v2.updated_at_utc, EXCLUDED.updated_at_utc),
            ...
            """;
        cmd.Parameters.AddWithValue("tenant", summary.TenantId);
        cmd.Parameters.AddWithValue("campaign", summary.CampaignId);
        cmd.Parameters.AddWithValue("revision", Math.Max(1, summary.Revision));
        cmd.Parameters.AddWithValue("status", TemporalBound(summary.Status, 32));
        cmd.Parameters.AddWithValue("severity", TemporalBound(summary.Severity, 32));
        ...
        AddNullableTimestamp(cmd, "tombstoned", summary.TombstonedAtUtc);
        ...
        cmd.Parameters.AddWithValue("title", TemporalBound(summary.Title, 300));
        cmd.Parameters.AddWithValue("summary", TemporalBound(summary.Summary, 2_000));
        await cmd.ExecuteNonQueryAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.Parameters.AddWithValue("tenant", TemporalTenant(tenantId));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            ...
            SELECT tenant_id, campaign_id, revision, status, severity, confidence,
                   first_observed_utc, last_observed_utc, updated_at_utc,
                   observation_count, episode_count, edge_count, contact_count, recurrence_count,
                   affected_asset_count, related_incident_count, related_incident_ids_json::text,
                   merged_into_campaign_id, tombstoned_at_utc,
                   involved_hosts_json::text, involved_ips_json::text, title, summary, snapshot_at_utc
            FROM ranked
            WHERE row_number=1
            ...
            """;
        cmd.Parameters.AddWithValue("tenant", TemporalTenant(tenantId));
        cmd.Parameters.AddWithValue("watermark", watermarkUtc);
        AddNullableTimestamp(cmd, "from", fromObservedAtUtc);
        AddNullableTimestamp(cmd, "to", toObservedAtUtc);
        ...
        AddNullableTimestamp(cmd, "after_last", afterLastObservedAtUtc);
    ...
    {
        await EnsureTemporalThreatSchemaAsync(cancellationToken);
        NormalizeTemporalObservation(observation);
        await using var conn = new NpgsqlConnection(_cs);
        await conn.OpenAsync(cancellationToken);
        await using var tx = await conn.BeginTransactionAsync(cancellationToken);

        await using (var campaignLock = conn.CreateCommand())
        {
            campaignLock.Transaction = tx;
            campaignLock.CommandText = "SELECT pg_advisory_xact_lock(hashtextextended(@scope, 0));";
            campaignLock.Parameters.AddWithValue("scope", $"{observation.TenantId}:{observation.CampaignId}");
            await campaignLock.ExecuteNonQueryAsync(cancellationToken);
        }
        ...
        await using (var insert = conn.CreateCommand())
        {
            ...
            insert.CommandText =
                """
                INSERT INTO threat_observations_v2(
                ...
                VALUES (@tenant, @campaign, @id, @episode, @contact, @kind,
                    @observed, @collected, @ingested, @occurrences, @incident, @payload::jsonb)
        ...
        if (!contactInserted) await UpdateContactAsync(conn, tx, observation, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            """
            INSERT INTO threat_candidate_observations_v2(
                tenant_id, observation_id, contact_id, context_key,
                observed_at_utc, ingested_at_utc, payload)
            VALUES (@tenant, @id, @contact, @context, @observed, @ingested, @payload::jsonb)
            ON CONFLICT(tenant_id, observation_id) DO NOTHING;
            """;
        ...
        cmd.Parameters.AddWithValue("observed", observation.ObservedAtUtc);
        cmd.Parameters.AddWithValue("ingested", observation.IngestedAtUtc);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            """
            SELECT payload::text FROM threat_candidate_observations_v2
        ...
        cmd.Parameters.AddWithValue("from", fromObservedAtUtc);
        cmd.Parameters.AddWithValue("to", toObservedAtUtc);
        cmd.Parameters.AddWithValue("take", Math.Clamp(take, 1, 501));
    ...
    {
        await EnsureTemporalThreatSchemaAsync(cancellationToken);
        membership.TenantId = TemporalTenant(membership.TenantId);
        membership.AssignedAtUtc = TemporalDate(membership.AssignedAtUtc, DateTimeOffset.UtcNow);
        ...
        await using (var campaignLock = conn.CreateCommand())
        {
            campaignLock.Transaction = tx;
            campaignLock.CommandText = "SELECT pg_advisory_xact_lock(hashtextextended(@scope, 0));";
            campaignLock.Parameters.AddWithValue("scope", $"{membership.TenantId}:{membership.CampaignId}");
            await campaignLock.ExecuteNonQueryAsync(cancellationToken);
        }
        var wasActive = false;
        await using (var existing = conn.CreateCommand())
        {
            ...
            wasActive = value is bool active && active;
        }
        ...
        if (membership.Active && !wasActive)
        {
            ...
            read.CommandText =
                """
                SELECT payload::text FROM threat_observations_v2
                WHERE tenant_id=@tenant AND observation_id=@observation
                  AND payload <> '{}'::jsonb LIMIT 1;
                """;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            ...
            UPDATE threat_campaigns_v2 SET
                ...
                tombstoned_at_utc=@tombstoned,
                updated_at_utc=GREATEST(updated_at_utc, @tombstoned),
                revision=revision + 1
        ...
        cmd.Parameters.AddWithValue("tombstoned", tombstonedAtUtc.ToUniversalTime());
        await cmd.ExecuteNonQueryAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            if (item is not null)
            {
                item.TenantId = TemporalTenant(tenantId);
        ...
        CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText =
            ...
            INNER JOIN threat_observation_memberships_v2 m
             ...
             AND m.campaign_id=@campaign AND m.active=TRUE
            WHERE o.tenant_id=@tenant AND o.ingested_at_utc <= @watermark
              AND (@details_only=FALSE OR o.payload <> '{}'::jsonb)
              AND (@from IS NULL OR o.observed_at_utc >= @from)
              AND (@to IS NULL OR o.observed_at_utc <= @to);
            """;
        AddTimelineParameters(cmd, tenantId, campaignId, watermarkUtc, fromObservedAtUtc,
            toObservedAtUtc, null, null, 1);
        cmd.Parameters.AddWithValue("details_only", detailsOnly);
        return Convert.ToInt64(await cmd.ExecuteScalarAsync(cancellationToken) ?? 0);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            """
            WITH bucketed AS (
                SELECT floor(extract(epoch FROM o.observed_at_utc) / @seconds)::bigint * @seconds AS bucket_epoch,
                       o.kind, SUM(o.occurrence_count)::bigint AS occurrence_count
        ...
        AddNullableTimestamp(cmd, "from", fromObservedAtUtc);
        AddNullableTimestamp(cmd, "to", toObservedAtUtc);
        cmd.Parameters.AddWithValue("after_epoch", NpgsqlDbType.Bigint,
            afterBucketStartUtc.HasValue
        ...
        cmd.CommandText =
            ...
            WITH filtered AS (
                SELECT o.contact_id,
                       ...
                       MAX(o.ingested_at_utc) AS updated_at_utc,
                       SUM(o.occurrence_count)::bigint AS observation_count,
                       COUNT(*)::bigint AS observation_record_count,
                       GREATEST(0, SUM(o.occurrence_count) - 1)::bigint AS recurrence_count,
                       SUM(CASE WHEN o.kind='authentication_failure' THEN o.occurrence_count ELSE 0 END)::bigint AS auth_failure_count,
                       SUM(CASE WHEN o.kind='authentication_success' THEN o.occurrence_count ELSE 0 END)::bigint AS auth_success_count,
                       SUM(CASE WHEN o.kind='network_open' THEN o.occurrence_count ELSE 0 END)::bigint AS open_count,
                       SUM(CASE WHEN o.kind='network_close' THEN o.occurrence_count ELSE 0 END)::bigint AS close_count,
            ...
            SELECT c.contact_id, c.source_node_id, c.destination_node_id, c.relation, c.technique, c.protocol,
                   ...
                   c.median_gap_seconds, c.p95_gap_seconds, c.beacon_score, c.evidence_refs_json::text,
                   c.sample_payload::text, c.tenant_id, c.campaign_id, f.provenance
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            """
            INSERT INTO temporal_correlation_outbox_v2(
                work_id, tenant_id, payload_json, enqueued_at_utc, attempts, next_attempt_utc)
            VALUES (@id, @tenant, @payload::jsonb, @enqueued, 0, @enqueued)
            ON CONFLICT(work_id) DO NOTHING;
            """;
        cmd.Parameters.AddWithValue("id", TemporalBound(item.WorkId, 128));
        cmd.Parameters.AddWithValue("tenant", TemporalTenant(item.TenantId));
        cmd.Parameters.AddWithValue("payload", item.PayloadJson);
        cmd.Parameters.AddWithValue("enqueued", TemporalDate(item.EnqueuedAtUtc, DateTimeOffset.UtcNow));
        await cmd.ExecuteNonQueryAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            """
            WITH picked AS (
                SELECT work_id FROM temporal_correlation_outbox_v2
                WHERE next_attempt_utc <= @now
                  AND (lease_until_utc IS NULL OR lease_until_utc <= @now)
                ORDER BY enqueued_at_utc ASC, work_id ASC
                LIMIT @take FOR UPDATE SKIP LOCKED
            )
            UPDATE temporal_correlation_outbox_v2 o SET
                lease_owner=@owner, lease_until_utc=@lease_until, attempts=o.attempts + 1
            FROM picked WHERE o.work_id=picked.work_id
            RETURNING o.work_id, o.tenant_id, o.payload_json::text, o.enqueued_at_utc,
                      o.attempts, o.lease_until_utc, o.lease_owner, o.last_error;
            """;
        var leaseUntil = nowUtc.Add(leaseDuration > TimeSpan.Zero ? leaseDuration : TimeSpan.FromMinutes(2));
        cmd.Parameters.AddWithValue("now", nowUtc);
        cmd.Parameters.AddWithValue("take", Math.Clamp(take, 1, 50));
        cmd.Parameters.AddWithValue("owner", TemporalBound(leaseOwner, 128));
        cmd.Parameters.AddWithValue("lease_until", leaseUntil);
        var items = new List<TemporalCorrelationWorkItem>();
        await using var reader = await cmd.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            items.Add(new TemporalCorrelationWorkItem
            {
                WorkId = reader.GetString(0),
                TenantId = reader.GetString(1),
                PayloadJson = reader.GetString(2),
                EnqueuedAtUtc = reader.GetFieldValue<DateTimeOffset>(3),
                Attempts = reader.GetInt32(4),
                LeaseUntilUtc = reader.IsDBNull(5) ? null : reader.GetFieldValue<DateTimeOffset>(5),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            ...
            """;
        cmd.Parameters.AddWithValue("retry", retryAtUtc);
        cmd.Parameters.AddWithValue("error", TemporalBound(error, 2_000));
        cmd.Parameters.AddWithValue("id", workId);
        cmd.Parameters.AddWithValue("owner", leaseOwner);
        await cmd.ExecuteNonQueryAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        cmd.CommandText =
            """
            DELETE FROM threat_episode_memberships_v2 em
            USING threat_observations_v2 o
            WHERE o.tenant_id=em.tenant_id AND o.observation_id=em.observation_id
              AND o.observed_at_utc < @aggregate;
            DELETE FROM threat_observation_memberships_v2 m
            USING threat_observations_v2 o
            WHERE o.tenant_id=m.tenant_id AND o.observation_id=m.observation_id
              AND o.observed_at_utc < @aggregate;
            UPDATE threat_observations_v2
                SET payload='{}'::jsonb, incident_id=NULL, campaign_id='', episode_id=NULL
                WHERE observed_at_utc < @detail AND payload <> '{}'::jsonb;
            ...
            UPDATE threat_contacts_v2 SET sample_payload='{}'::jsonb, evidence_refs_json='[]'::jsonb
            ...
            """;
        cmd.Parameters.AddWithValue("detail", detailBeforeUtc);
        cmd.Parameters.AddWithValue("aggregate", aggregateBeforeUtc);
    ...
    private const string TemporalSummarySelect =
        ...
        SELECT tenant_id, campaign_id, revision, status, severity, confidence,
               ...
               affected_asset_count, related_incident_count, related_incident_ids_json::text,
               merged_into_campaign_id, tombstoned_at_utc,
               involved_hosts_json::text, involved_ips_json::text, title, summary, updated_at_utc
        FROM threat_campaigns_v2
        """;

    private static ThreatCampaignV2Summary ReadTemporalSummary(NpgsqlDataReader reader) => new()
    {
        ...
        FirstObservedAtUtc = reader.GetFieldValue<DateTimeOffset>(6),
        LastObservedAtUtc = reader.GetFieldValue<DateTimeOffset>(7),
        ...
        TombstonedAtUtc = reader.IsDBNull(18) ? null : reader.GetFieldValue<DateTimeOffset>(18),
        ...
        SnapshotAtUtc = reader.GetFieldValue<DateTimeOffset>(23)
    };

    private static ThreatContactAggregate ReadContact(NpgsqlDataReader reader)
    {
        ...
        return new ThreatContactAggregate
        {
            ...
            FirstObservedAtUtc = reader.GetFieldValue<DateTimeOffset>(8),
            LastObservedAtUtc = reader.GetFieldValue<DateTimeOffset>(9),
        ...
        CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText =
            """
            INSERT INTO threat_contacts_v2(
            ...
            VALUES (@tenant, @campaign, @contact, @source, @destination,
                ...
                NULL, NULL, 0, '[]'::jsonb, @evidence_refs::jsonb, @payload::jsonb)
        ...
        CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText =
            ...
            UPDATE threat_contacts_v2 SET
                first_observed_utc=LEAST(first_observed_utc, @observed),
                last_observed_utc=GREATEST(last_observed_utc, @observed),
                updated_at_utc=GREATEST(updated_at_utc, @updated),
                ...
                inferred=inferred AND @inferred,
                ...
                gap_samples_json=@gap_samples::jsonb,
                evidence_refs_json=@evidence_refs::jsonb,
                sample_payload=CASE WHEN @observed >= last_observed_utc
                    THEN @payload::jsonb ELSE sample_payload END
        ...
        CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText =
            ...
            VALUES (@tenant, @campaign, 1, 'Open', 'Informational',
                ...
                @contact_delta, @contact_delta, @recurrences, '[]'::jsonb, '[]'::jsonb, '', '')
            ON CONFLICT(tenant_id, campaign_id) DO UPDATE SET
                revision=threat_campaigns_v2.revision + 1,
                first_observed_utc=LEAST(threat_campaigns_v2.first_observed_utc, EXCLUDED.first_observed_utc),
                last_observed_utc=GREATEST(threat_campaigns_v2.last_observed_utc, EXCLUDED.last_observed_utc),
        ...
        cmd.Parameters.AddWithValue("observed", observation.ObservedAtUtc);
        cmd.Parameters.AddWithValue("updated", observation.IngestedAtUtc);
    ...
    private static void AddObservationParameters(NpgsqlCommand cmd, ThreatObservation observation)
    {
        cmd.Parameters.AddWithValue("tenant", observation.TenantId);
        cmd.Parameters.AddWithValue("campaign", observation.CampaignId);
        cmd.Parameters.AddWithValue("id", observation.ObservationId);
        ...
        cmd.Parameters.AddWithValue("observed", observation.ObservedAtUtc);
        AddNullableTimestamp(cmd, "collected", observation.CollectedAtUtc);
        cmd.Parameters.AddWithValue("ingested", observation.IngestedAtUtc);
        ...
        CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText =
            ...
            """;
        cmd.Parameters.AddWithValue("tenant", TemporalTenant(membership.TenantId));
        ...
        cmd.Parameters.AddWithValue("assigned", membership.AssignedAtUtc);
        cmd.Parameters.AddWithValue("confidence", Math.Clamp(membership.Confidence, 0, 1));
        cmd.Parameters.AddWithValue("provenance", TemporalBound(membership.Provenance, 128));
        cmd.Parameters.AddWithValue("active", membership.Active);
        await cmd.ExecuteNonQueryAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (var membership = conn.CreateCommand())
        {
            ...
            membership.Parameters.AddWithValue("assigned", observation.IngestedAtUtc);
    ...
    private static string NewEpisodeId(string observationId)
    {
        var suffix = observationId.StartsWith("observation-", StringComparison.Ordinal)
            ...
            : observationId;
        return "episode-" + TemporalBound(suffix, 56);
    }
    ...
    private static void AddContactParameters(NpgsqlCommand cmd, ThreatObservation observation)
    {
        ...
        cmd.Parameters.AddWithValue("inferred", observation.Inferred);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (var read = conn.CreateCommand())
        {
            ...
            read.CommandText =
                """
                SELECT evidence_refs_json::text
        ...
        await using (var chronological = conn.CreateCommand())
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
                timestamps.Add(reader.GetFieldValue<DateTimeOffset>(0));
        }
        timestamps.Sort();
        ...
        int take)
    {
        ...
        AddNullableTimestamp(cmd, "after_time", after);
        cmd.Parameters.AddWithValue("after_id", afterId ?? string.Empty);
        cmd.Parameters.AddWithValue("take", Math.Clamp(take, 1, 501));
    }

    private static void AddNullableTimestamp(NpgsqlCommand cmd, string name, DateTimeOffset? value) =>
        cmd.Parameters.AddWithValue(name, NpgsqlDbType.TimestampTz, value.HasValue ? value.Value : DBNull.Value);

    private static void NormalizeTemporalObservation(ThreatObservation observation)
    {
        observation.TenantId = TemporalTenant(observation.TenantId);
        observation.ObservedAtUtc = TemporalDate(observation.ObservedAtUtc, DateTimeOffset.UtcNow);
        observation.IngestedAtUtc = TemporalDate(observation.IngestedAtUtc, DateTimeOffset.UtcNow);
        ...
        observation.Kind = TemporalBound(observation.Kind, 64);
        observation.Relation = TemporalBound(observation.Relation, 64);
    }

    private static DateTimeOffset TemporalDate(DateTimeOffset value, DateTimeOffset fallback) =>
        value == default || value == DateTimeOffset.MinValue ? fallback.ToUniversalTime() : value.ToUniversalTime();

    private static string TemporalTenant(string? tenantId) =>
        string.IsNullOrWhiteSpace(tenantId) ? "default" : tenantId.Trim().ToLowerInvariant();

    private static string TemporalBound(string? value, int maxLength)
    {
