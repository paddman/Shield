...
;   NTShield-Agent-Setup.exe /VERYSILENT /CentralUrl=https://10.0.0.5:7443
...
[Setup]
; Separate product from full stack installer
...
SetupLogging=yes
; Force-close apps that lock files; we also hard-kill Agent/Tray in PrepareToInstall
...
FinishedLabel=NT Shield Agent ติดตั้งเรียบร้อยแล้ว!%n%nService: NTShieldAgent%n%nตรวจเวอร์ชัน: เปิด %ProgramFiles%\NT Shield Agent\VERSION.txt%nหรือ status.json ต้องเป็น {#MyAppVersion}%n%nตรวจว่า Central ทำงาน แล้วเปิด Dashboard ชี้ URL เดียวกัน
...
Filename: "powershell.exe"; \
  Parameters: "-NoProfile -ExecutionPolicy Bypass -File ""{app}\Installer\register-agent-service.ps1"" -InstallDir ""{app}"" -DataDir ""{commonappdata}\NTShield\Agent"" -ServiceName ""NTShieldAgent"" -StartService {code:StartServiceFlag} -CentralUrl ""{code:GetCentralUrl}"" -EnrollmentToken ""{code:GetEnrollmentToken}"""; \
...
begin
  ...
  begin
    ...
    if (Pos('http://', LowerCase(H)) = 1) or (Pos('https://', LowerCase(H)) = 1) then
    begin
      U := H;
...
// Hard-stop service + kill Agent/Tray before files are replaced (in-place upgrade).
// Prevents "applications are using files that need to be updated" dialog when possible.
procedure KillAgentProcesses;
var
  ResultCode: Integer;
begin
  // Best effort - never abort setup on kill failure
...
begin
  ...
  // 1) Quick taskkill / sc stop
  KillAgentProcesses;

  // 2) Full PowerShell helper (waits for service to fully stop)
...
begin
  Result :=
    ...
    Space + GetCentralUrl('') + NewLine + NewLine +
    'Service name: NTShieldAgent' + NewLine +
    'Data: %ProgramData%\NTShield\Agent' + NewLine + NewLine +
    'Upgrade: existing Agent service / tray will be stopped and killed before files are replaced.' + NewLine +
    'Note: Open Dashboard Settings and set the same Central URL to see this host.' + NewLine +
    MemoTasksInfo;
end;
